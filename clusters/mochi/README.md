...
The `mochi` cluster is a fully operational Confluent Platform + Flink deployment on AWS EKS with private-only access.
...
**Domain**: `*.mochi.platform.dspdemos.com`
...
See [`terraform/mochi/README.md`](../../terraform/mochi/README.md) for full variable reference and outputs.
...
aws eks update-kubeconfig --region us-east-1 --name mochi

# Start the SOCKS5 tunnel (keep this running in a separate terminal)
aws ssm start-session \
  --region us-east-1 \
  --target $(terraform -chdir=terraform/mochi output -raw bastion_instance_id) \
...
terraform -chdir=terraform/mochi output ebs_csi_driver_role_arn
terraform -chdir=terraform/mochi output cert_manager_role_arn
terraform -chdir=terraform/mochi output external_dns_role_arn
terraform -chdir=terraform/mochi output aws_lb_controller_role_arn
```

Update the placeholder in `infrastructure/aws-ebs-csi-driver/overlays/mochi/values.yaml`:
...
```bash
kubectl apply -f clusters/mochi/bootstrap.yaml
```
...
<!-- Content in this section intentionally duplicated between this README.md and `terraform/mochi/README.md` -->
...
6. In the FoxyProxy toolbar icon, select **Proxy by Patterns** (or enable the `mochi SOCKS5` proxy directly).
...
**ArgoCD UI:**
- **URL**: https://argocd.mochi.platform.dspdemos.com
- **Username**: `admin`
- **Password**: `kubectl -n argocd get secret argocd-initial-admin-secret -o jsonpath="{.data.password}" | base64 -d`

**Control Center:**
- **URL**: https://controlcenter.mochi.platform.dspdemos.com

**Grafana:**
- **URL**: https://grafana.mochi.platform.dspdemos.com
- **Username**: `admin`
- **Password**: `prom-operator`

**Prometheus:**
- **URL**: https://prometheus.mochi.platform.dspdemos.com

**Alertmanager:**
- **URL**: https://alertmanager.mochi.platform.dspdemos.com

**CMF API:**
- **URL**: https://cmf.mochi.platform.dspdemos.com

**MinIO Console:**
- **URL**: https://s3-console.mochi.platform.dspdemos.com

**Keycloak** *(added in Task N)*:
- **URL**: https://keycloak.mochi.platform.dspdemos.com
...
Defined in `clusters/mochi/infrastructure/kustomization.yaml`:
...
Defined in `clusters/mochi/workloads/kustomization.yaml`:
...
```bash
ls -la ./clusters/mochi/infrastructure/
ls -la ./clusters/mochi/workloads/
```
...
```bash
./scripts/validate-cluster.sh mochi --verbose
```
...
```bash
terraform -chdir=terraform/mochi destroy
terraform -chdir=terraform/dns-bootstrap destroy
```

> [!WARNING]
> `terraform destroy` on `mochi` deletes the EKS cluster, all node groups, the VPC, bastion, and IRSA roles. Back up or snapshot any persistent data (EBS volumes from PVCs) before destroying.
