...
### Added
- **flink-demo-rbac-mtls — Schema Registry HTTPS** ([#275](https://github.com/osowski/confluent-platform-gitops/issues/275)): the Schema Registry REST API now serves over HTTPS (cert-manager server cert); Control Center connects to it over TLS. Component→Kafka authentication stays OAuth (CFK RBAC carries component identity via the OAuth/MDS token, not a client cert). Adds `scripts/validate-mtls.sh` to validate the live mTLS posture.
