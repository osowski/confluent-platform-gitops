...
- **CMF's MDS authorization leg now mTLS-backed on `flink-demo-rbac`** ([#412](https://github.com/osowski/confluent-platform-gitops/issues/412)): CMF authenticates to MDS via a cert-manager-issued client certificate (`User:cmf`, no new RBAC bindings needed) instead of Keycloak OAuth — the only MDS client using mTLS instead of the LDAP-backed bearer chain (#411), per the epic's documented CMF-chart limitation. CMF's own end-user login and CMFRestClass remain Keycloak-backed pending [#418](https://github.com/osowski/confluent-platform-gitops/issues/418).
