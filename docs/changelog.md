...
- **colors/shapes JAR autoscaler jobs no longer crash-loop on Kafka SASL on `flink-demo-rbac`** ([#421](https://github.com/osowski/confluent-platform-gitops/issues/421)): its internal listener has been LDAP-only since #410, but these jobs still tried OAUTHBEARER. A new `rbac-ldap` component switches them to LDAP-backed SASL/PLAIN and HTTP Basic; OAuth clusters are unchanged.
