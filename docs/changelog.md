...
### Added
- **`cp-flink-validation` Claude Skill** ([#424](https://github.com/osowski/confluent-platform-gitops/issues/424)): a conversational skill that interviews a user to define a test case (health, data-flow, resource-state, or REST-response assertions) against `flink-demo`, then runs it via either a GitOps branch/retarget/sync flow or a direct kubectl/CFK/CMF-REST flow. v1 is `flink-demo`-only with no automated teardown.
