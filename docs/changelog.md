...
### Added
- **Headlamp Kubernetes dashboard on all clusters** ([#148](https://github.com/osowski/confluent-platform-gitops/issues/148)): token login by default; Keycloak OIDC SSO on RBAC clusters (flink-demo-rbac, flink-demo-rbac-mtls, eks-demo). Reachable at `headlamp.<cluster>.<domain>`; new clusters get token-based login automatically via `new-cluster.sh`.
