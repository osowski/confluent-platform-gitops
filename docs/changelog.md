...
### Added
- **Cross-CA truststore exchange spike (ADR-0012)** ([#395](https://github.com/osowski/confluent-platform-gitops/issues/395)): reproduced the real customer scenario Epic #377 was created for — a Flink client cert and Kafka broker cert signed by independent CAs. An untrusted CA fails opaquely on the client (`Failed to process post-handshake messages`) and **silently even at broker DEBUG**; confirmed live that a *different-but-explicitly-trusted* client CA works, including through an intermediate covering multiple leaf certs with zero added Kafka broker config. No permanent GitOps changes; all spike resources deleted.
