...
### Added
- **Cross-CA truststore exchange spike (ADR-0012)** ([#395](https://github.com/osowski/confluent-platform-gitops/issues/395)): reproduced the real customer scenario Epic #377 was created for — a Flink client cert and Kafka broker cert signed by independent CAs. Confirmed the client-trust failure is clear (`PKIX path building failed`) but the client-cert-trust failure is opaque on the client (`Failed to process post-handshake messages`) and **silent even at DEBUG** on the broker; no GitOps changes, spike resources deleted.
