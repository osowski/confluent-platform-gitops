...
### Added
- **flink-demo-rbac — Flink SQL statement pipeline** ([#158](https://github.com/osowski/confluent-platform-gitops/issues/158)): standalone RBAC-secured Flink SQL `INSERT INTO` statement on dedicated `shapes-sql-input`/`shapes-sql-output` topics, created via the CMF Statements API by the shapes init job. Coexists with the JAR FlinkApplications; scale `shapes-sql-producer` to 1 to feed it. Requires CMF chart 2.3.1; Flink SQL image bumped to `cp-flink-sql:1.19-cp8`.
