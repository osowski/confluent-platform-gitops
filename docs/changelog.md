...
### Fixed
- **`confluent-resources` no longer falsely reports OutOfSync on whole-CPU values** ([#352](https://github.com/osowski/confluent-platform-gitops/issues/352)): Kubernetes canonicalizes an exact-multiple-of-`1000m` `cpu` quantity to its decimal-core form, so an unquoted value in git never matched the live object's string. Quoted (`"1"`, `"2"`) across all affected overlays fleet-wide.
