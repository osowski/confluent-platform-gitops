...
### Added
- **flink-demo-rbac-mtls — Schema Registry mTLS + HTTPS** ([#275](https://github.com/osowski/confluent-platform-gitops/issues/275)): new dedicated `mtls-clients` Kafka listener (cert-authenticated on-ramp for service accounts); Schema Registry now authenticates to Kafka with its own `CN=sr` certificate (`User:sr` RBAC bindings) and serves its REST API over HTTPS. Adds `scripts/validate-mtls.sh` to validate the full mTLS posture of a live cluster.
