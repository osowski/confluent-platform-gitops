...
All PKI on this cluster, including the `secure-sql` mTLS tenant delivered under Epic #377, shares a single CA (`rbac-mtls-ca-issuer`) — it never exercises a Flink client and a Kafka endpoint whose certificates are signed by independent CAs, the actual customer-reported scenario. [ADR-0012](../adrs/0012-cross-ca-truststore-exchange-flink-kafka.md) records a spike that reproduces both the failure and the working fix live: a client certificate's CA does not need to match the broker's server CA, as long as the broker's listener truststore trusts that CA (or, for many clients, one shared issuing/intermediate CA covering all of them).
