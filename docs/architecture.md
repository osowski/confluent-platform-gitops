...
- MinIO provides S3-compatible storage for Flink checkpoints/savepoints, plus a dedicated `artifacts` bucket (`basePath: s3://artifacts/cmf`) backing CMF 2.4.x artifact management, with credentials injected via `extraEnv` from the reflected `minio-credentials` secret
...
`flink-demo-rbac-mtls` forks `flink-demo-rbac` to add mTLS on the Kafka↔KRaft controller and inter-broker replication paths, plus a dedicated client-facing listener for one Flink SQL tenant — see the cluster's own [README](../clusters/flink-demo-rbac-mtls/README.md) for the PKI and listener configuration. **This is still mostly a work in progress**: service-account and client authentication for CMF, Control Center, producers, and the `shapes`/`colors` Flink SQL tenants still go through Keycloak OIDC/SSO exactly as on `flink-demo-rbac`. The `secure-sql` tenant (see [ADR-0011](../adrs/0011-flink-sql-kafka-mtls-cert-manager.md)) is the first exception: its Flink SQL statement connects to Kafka over a cert-manager-issued mTLS client certificate instead — including CMF's own control-plane pod, which needs the same certificate to compile statements against this tenant's database (accepted for this experiment only; see the ADR). The same [Multi-Tenant RBAC Architecture](#multi-tenant-rbac-architecture-flink-demo-rbac-cluster) breakdown applies unchanged for everything else client-facing.
